    ...
    then
        ...
        then
            ...
            then
                ...
                status=$?
                print_message " При запуске упали ваши тесты для проекта SAFNote. Проверьте тесты этого проекта " "=" 1
                echo \`\`\` 1>&2
                exit $status
            fi
        else
            status=$?
            print_message " При запуске упали ваши тесты для проекта SAFNews. Проверьте тесты этого проекта " "=" 1
            echo \`\`\` 1>&2
            exit $status
        fi
    else
